...
def fun(item):
    ...
    if item[0]<'Q':
        return 1
    return 2
#funs uses index and passes it to fun.
...
```python
df.groupby('STNAME').agg({'CENSUS2010POP': np.average}) # here column name is the same as target name
```
...
pd.cut(df['avg'],10) # intervals by 10 category.
```
...
# This is called cuting data in 3.
```
...
```python
ts3.index = pd.to_datetime(ts3.index) #pandas convert to daytime index
ts3
```
...
```python
df.diff() # difference between previous record in row in the same column.
```
...
```python
df.resample('M').mean() #mean for each month 'M'
```
...
```python
df.asfreq('W', method='ffill') # for each week and method = 'ffill' because we obtain missing values
```
...
```
    ...
    <matplotlib.axes._subplots.AxesSubplot at 0x7f8cbb896d68>
